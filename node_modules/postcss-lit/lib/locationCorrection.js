...
const correctLocation = (node, loc, baseIndentations, prefixOffsets) => {
    ...
    for (let i = 0; i < node.quasi.expressions.length; i++) {
        ...
        if (expr &&
            ...
            const placeholderSize = (0, util_js_1.createPlaceholder)(i).length;
...
function locationCorrectionWalker(expr) {
    return (node) => {
        ...
        if (node.source?.start) {
            node.source.start = correctLocation(expr, node.source.start, baseIndentations, root.raws['litPrefixOffsets']);
        }
        if (node.source?.end) {
            node.source.end = correctLocation(expr, node.source.end, baseIndentations, root.raws['litPrefixOffsets']);
        }
    };
}
